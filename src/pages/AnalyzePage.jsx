...
const STEPS = [
  { label: '🖼️  Checking image fingerprints…',  ms: 400  },
  { label: '💰  Comparing price to market data…', ms: 800  },
  { label: '🔍  Scanning for scam keywords…',     ms: 1200 },
  { label: '📋  Running duplicate detection…',    ms: 1600 },
  { label: '📞  Verifying seller contact…',       ms: 2000 },
  { label: '💬  Analyzing conversation text…',    ms: 2400 },
  { label: '🧠  Calculating final scam score…',   ms: 2800 },
]

function downloadPDF(result, formTitle) {
  // Build a printable HTML and trigger browser print
  ...
  const html = `
    ...
    <body>
      ...
      <div class="sub">Listing: ${formTitle || 'Untitled'} &nbsp;·&nbsp; Scan ID: ${result.scanId} &nbsp;·&nbsp; ${new Date(result.scannedAt).toLocaleString()}</div>
      <div class="score">${result.scamScore}<span class="risk">${result.riskLevel} Risk</span></div>
      <p style="margin-top:8px;font-size:13px;color:#555">Confidence: <strong>${result.confidence}%</strong> &nbsp;·&nbsp; Scam Type: <strong>${SCAM_LABELS[result.scamType?.type] || 'Unknown'}</strong></p>
      ...
      <div class="footer">Generated by GhostRent · ghostrent.app · Report is based on heuristic analysis. Always verify independently.</div>
    </body>
    </html>
  `
